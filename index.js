...
function checkArray(obj) {
  ...
  //Повертаємо false
  if (Array.isArray(obj) && obj !== null && obj !== undefined) {
    return true;
...
function createArray(...elements) {
  ...
  console.log("Помилка! Пустий масив.");
  return [];
}
...
function findElementIndexes(arr, element) {
  ...
  if (Array.isArray(arr)) {
    ...
    if (firstIndex != -1) {
      result[result.length] = firstIndex;
    }
    if (lastIndex != -1 && firstIndex != lastIndex) {
      result[result.length] = lastIndex;
    }
  }
  return result;
}
